...
struct AgentSession: Codable, Identifiable, Hashable {
    ...
    var id: String { (machine.map { "remote:" + $0.routeIdentity + ":" } ?? "local:") + terminalID + ":" + paneID }
    ...
    static func decodeList(_ data: Data, machine: HerdrMachine? = nil) throws -> [AgentSession] {
    ...
    static func sorted(_ agents: [AgentSession]) -> [AgentSession] {
        agents.sorted {
