...
@objc protocol VolantAIHostProtocol {
...
enum AIConnectionKind: String, Codable, CaseIterable, Identifiable {
    case acp, byok, local
    var id: String { rawValue }
    var title: String { switch self { case .acp: return "ACP"; case .byok: return "BYOK"; case .local: return "Local Models" } }
}
enum AIAPIProvider: String, Codable, CaseIterable, Identifiable {
    case openAI, anthropic, compatible
    var id: String { rawValue }
    var title: String { switch self { case .openAI: return "OpenAI"; case .anthropic: return "Anthropic"; case .compatible: return "OpenAI-compatible" } }
    var endpoint: String { switch self { case .openAI: return "https://api.openai.com/v1"; case .anthropic: return "https://api.anthropic.com/v1"; case .compatible: return "" } }
}
struct AIHTTPConfiguration: Codable, Equatable {
    var provider: AIAPIProvider = .openAI
    var endpoint = "https://api.openai.com/v1"
    var model = ""
    var local = false
    var credentialID: String { "\(provider.rawValue):\(endpoint.trimmingCharacters(in: CharacterSet(charactersIn: "/")))" }
...
enum AIHTTPError: Error, LocalizedError {
...
struct AILocalServer: Identifiable, Equatable {
    var name: String
    var endpoint: String
    var models: [String] = []
    var message: String = "Checking…"
    var id: String { endpoint }
    var configuration: AIHTTPConfiguration { AIHTTPConfiguration(provider: .compatible, endpoint: endpoint, local: true) }
