...
final class ExtensionManager: NSObject, VeyCapabilityClientProtocol {
    private(set) var extensions: [InstalledExtension] = []
    private var active: ExtensionManifest?
    var onLog: (String) -> Void = { NSLog("[vey-ext] %@", $0) }
    ...
    func run(_ ext: InstalledExtension, input: String, completion: @escaping (Result<String, Error>) -> Void) {
        ...
        connection.remoteObjectInterface = NSXPCInterface(with: VeyExtensionHostProtocol.self)
        connection.exportedInterface = NSXPCInterface(with: VeyCapabilityClientProtocol.self)
        connection.exportedObject = self
        connection.resume()
        let proxy = connection.remoteObjectProxyWithErrorHandler { error in
            DispatchQueue.main.async { completion(.failure(error)) }
            connection.invalidate()
        } as? VeyExtensionHostProtocol
