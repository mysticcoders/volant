...
enum Backup {
    static func export() {
        ...
        panel.prompt = "Export Here"; panel.message = "Choose a folder. Vey will create a dated backup folder inside it."
        guard panel.runModal() == .OK, let base = panel.url else { return }
        let stamp = Date().formatted(.iso8601.year().month().day())
        let dest = base.appendingPathComponent("Vey-Backup-\(stamp)", isDirectory: true)
    ...
    static func importBackup() -> Bool {
        ...
        panel.prompt = "Import"; panel.message = "Choose a Vey-Backup folder."
