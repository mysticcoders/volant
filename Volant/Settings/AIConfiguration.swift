...
struct AIConfiguration: Codable, Equatable {
    var provider = ""
    var connection: AIConnectionKind = .acp
    var api = AIHTTPConfiguration()
    var localAPI = AIHTTPConfiguration(provider: .compatible, endpoint: "http://127.0.0.1:11434/v1", local: true)
    var http: AIHTTPConfiguration { connection == .local ? localAPI : api }
    var isConfigured: Bool { connection == .acp ? ACPProvider(rawValue: provider) != nil : (try? http.validate()) != nil }
    var project = ""
    enum CodingKeys: String, CodingKey { case provider, project, connection, api, localAPI }
    init() {}
