...
Live isolated preview verified Return starts the fake session, the cup exposes remaining time through native help/accessibility, and clicking the cup stops it and removes the control. Removed the old success feedback text too: it consumed a row and could remain after the cup stopped the session. Errors remain visible.
