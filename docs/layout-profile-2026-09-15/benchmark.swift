...
print("mode,sample,request_to_window_server_visible_ms,request_unix_seconds,pid")
    ...
    for trial in 1...count {
        ...
        pump(0.3)
        let requestWallTime = Date().timeIntervalSince1970
        ...
        while Date() < deadline {
            ...
            if let app = result, visible(app.processIdentifier) {
                ...
                print("\(mode),\(trial),\(String(format: "%.3f", (ProcessInfo.processInfo.systemUptime - timestamp) * 1000)),\(String(format: "%.6f", requestWallTime)),\(app.processIdentifier)")
                break
            }
