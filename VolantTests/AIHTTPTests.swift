...
final class AIHTTPTests: XCTestCase {
    func testEndpointValidationAndCredentialIsolation() throws {
        for endpoint in ["http://example.com/v1", "https://name:secret@example.com/v1", "https://example.com/v1?key=secret", "https://example.com/v1#fragment"] {
            XCTAssertThrowsError(try Volant.AIHTTPConfiguration(provider: .compatible, endpoint: endpoint).validate(requireModel: false))
        }
        for endpoint in ["http://localhost:1234/v1", "http://127.0.0.1:11434/v1", "http://[::1]:1234/v1"] {
            XCTAssertNoThrow(try Volant.AIHTTPConfiguration(provider: .compatible, endpoint: endpoint, local: true).validate(requireModel: false))
        }
        XCTAssertThrowsError(try Volant.AIHTTPConfiguration(provider: .compatible, endpoint: "https://example.com/v1", local: true).validate(requireModel: false))
        XCTAssertThrowsError(try Volant.AIHTTPConfiguration(provider: .openAI, endpoint: "https://example.com/v1").validate(requireModel: false))
        let first = Volant.AIHTTPConfiguration(provider: .compatible, endpoint: "https://one.example/v1")
        let second = Volant.AIHTTPConfiguration(provider: .compatible, endpoint: "https://two.example/v1")
        XCTAssertNotEqual(first.credentialID, second.credentialID)
    }
    func testProviderRequestShapesAndNoTools() throws {
        let transport = Volant.AIHTTPTransport()
        let messages = [Volant.ACPMessage(role: "You", text: "Fictional question"), Volant.ACPMessage(role: "Agent", text: "Answer")]
        for provider in Volant.AIAPIProvider.allCases {
            let config = Volant.AIHTTPConfiguration(provider: provider, endpoint: provider == .compatible ? "https://test.example/v1" : provider.endpoint, model: "fixture-model")
        ...
        let local = Volant.AIHTTPConfiguration(provider: .compatible, endpoint: "http://127.0.0.1:1234/v1", model: "fixture", local: true)
        XCTAssertNil(try transport.chatRequest(local, key: "", messages: messages).value(forHTTPHeaderField: "Authorization"))
        XCTAssertThrowsError(try transport.chatRequest(Volant.AIHTTPConfiguration(model: "fixture"), key: "", messages: messages))
    }
    func testStreamingFramesUnicodeAndProviderCompletion() throws {
        let fixtures: [(Volant.AIAPIProvider, String)] = [
        ...
        for (provider, wire) in fixtures {
            var decoder = Volant.AIStreamDecoder(provider: provider), text = ""
    ...
    func testMalformedTruncatedOversizedAndUnsupportedStreams() throws {
        var incomplete = Volant.AIStreamDecoder(provider: .compatible)
        ...
        for frame in ["data: not json\n\n", "data: {\"error\":{}}\n\n", "data: {\"choices\":[{\"delta\":{\"tool_calls\":[]}}]}\n\n", "data: {\"choices\":[{\"finish_reason\":\"length\"}]}\n\n", String(repeating: "x", count: 65_537)] {
            var decoder = Volant.AIStreamDecoder(provider: .compatible)
