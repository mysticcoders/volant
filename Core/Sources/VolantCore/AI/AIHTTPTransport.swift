...
public final class AIRedirectGuard: NSObject, URLSessionTaskDelegate {
    public func urlSession(_ session: URLSession, task: URLSessionTask, willPerformHTTPRedirection response: HTTPURLResponse,
                    newRequest request: URLRequest, completionHandler: @escaping (URLRequest?) -> Void) { completionHandler(nil) }
}
public struct AIStreamDecoder {
    public var provider: AIAPIProvider
    private var line = Data()
    private var fields: [String] = []
    private var eventBytes = 0
    private var total = 0
    public private(set) var finished = false
    public init(provider: AIAPIProvider) { self.provider = provider }
...
public final class AIHTTPTransport {
    ...
    public func request(_ config: AIHTTPConfiguration, key: String, path: String) throws -> URLRequest {
    ...
    public func chatRequest(_ config: AIHTTPConfiguration, key: String, messages: [ACPMessage]) throws -> URLRequest {
        try config.validate()
    ...
    public func stream(_ config: AIHTTPConfiguration, key: String, messages: [ACPMessage], delta: @escaping (String) async throws -> Void) async throws {
    ...
    public func models(_ config: AIHTTPConfiguration, key: String) async throws -> [String] {
