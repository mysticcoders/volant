...
- Spotlight start refusal must resolve file-search callbacks and expose scoped Retry without discarding existing app results. Cancel old queries before debouncing replacements. Unreadable notes retain their IDs/pins and a read-only placeholder; never save empty fallback text over their files. Directory read failures preserve the prior list and dirty edits. See docs/search-note-recovery.md.
